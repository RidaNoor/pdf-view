<?php
/*
Plugin Name: PDF View Generator
*/

define('PDFView_PLUGIN_VERSION', '1.8.5');

function PDFView_plugin_parse_request($wp) {
    try {
    if (array_key_exists('PDFView-lite-plugin', $wp->query_vars)
            && $wp->query_vars['PDFView-lite-plugin'] == 'get-pdf') {
          ...
          if($_SERVER['REQUEST_METHOD']=='HEAD'){
              ...
              foreach ($browserRequestHeaders as $name => $value) {
                if(strpos($name,'Origin') !== false && strpos($value,'PDFView.com') !== false){
                      $hasOrigin = true;
                }
              }
              ...
              if(!empty($_SERVER['HTTPS'])){
                  header('Access-Control-Allow-Origin: https://PDFView.com');
              }else{
                  header('Access-Control-Allow-Origin: http://PDFView.com');
              }
...
function PDFView_plugin_add_shortcode_cb( $atts, $content="" ) {
	$defaults = array(
		'width' => '100%',
		'height' => '500',
		'scrolling' => 'no',
	...
	$html           = "\n".'<!-- PDFView PDF flipbook plugin v.'.PDFView_PLUGIN_VERSION.' ."\n";
	$html          .= '<iframe title="PDFView website pdf viewer"';
  $pdfUrl         = "";
  $theme          = "";
  $thumbs         = true;
  $title          = "";
  ...
  $cloudhosted    = false;
	...
	foreach( $atts as $attr => $value ) {
        ...
        if(strpos($value,'publ.PDFView.com') > 0 || strpos($value,'online.PDFView.com') > 0){
            ...
            $pdfUrl = 'http://PDFView.com/flipbook/?pdf=' . esc_attr( $value ) . (strpos(esc_attr($value),WP_CONTENT_URL)===0?"?wp-hosted=1":"");

    				if(!empty($_SERVER['HTTPS'])){ // use https if PDF is on a https url
                $pdfUrl = 'https://PDFView.com/flipbook/?pdf=' . esc_attr( $value ) . (strpos(esc_attr($value),WP_CONTENT_URL)===0?"?wp-hosted=1":"");
    				}
        }
...
add_shortcode( 'flipbook', 'PDFView_plugin_add_shortcode_cb' );
add_action('parse_request', 'PDFView_plugin_parse_request');

function PDFView_lite_queryvars($vars) {
    $vars[] = 'PDFView-lite-plugin';
    return $vars;
}
add_filter('query_vars', 'PDFView_lite_queryvars');

function PDFView_plugin_row_meta_cb( $links, $file ) {
	if ( $file == plugin_basename( __FILE__ ) ) {
		$row_meta = array(
			'support' => '<a href="https://PDFView.com/questions-and-answers.jsp" target="_blank"><span class="dashicons dashicons-editor-help"></span> ' . __( 'PDFView', 'PDFView' ) . '</a>',
			'PDFView Publisher' => '<a href="https://PDFView.com/" target="_blank"><span class="dashicons dashicons-star-filled"></span> ' . __( 'PDFView Publisher', 'PDFView' ) . '</a>'
		);
		$links = array_merge( $links, $row_meta );
	}
	return (array) $links;
}
add_filter( 'plugin_row_meta', 'PDFView_plugin_row_meta_cb', 10, 2 );
